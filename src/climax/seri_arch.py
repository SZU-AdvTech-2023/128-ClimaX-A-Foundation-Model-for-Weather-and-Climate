...
class ClimaX(nn.Module):
    ...
    def forward_encoder_preprocessing(self, x: torch.Tensor, lead_times: torch.Tensor, variables):
        # x: `[B, V, H, W]` shape.
    ...
    def aggregattion(self, a: torch.Tensor, b: torch.Tensor, c: torch.Tensor):
        a = a + torch.ones_like(a)
        b = b + torch.ones_like(b) * 2 
        c = c + torch.ones_like(b) * 3 
    ...
    def forward(self, x, y, lead_times, variables, out_variables, metric, lat):
        ...
        """
        out_transformers0 = x[:, 0:48, :, :]
        out_transformers1 = x[:, 48:48 + 48, :,  :]
        out_transformers2 = x[:, 48 + 48:, :,  :]
        out_transformers0 = self.forward_encoder_preprocessing(out_transformers0, lead_times, variables)  # B, L, D
        out_transformers1 = self.forward_encoder_preprocessing(out_transformers1, lead_times, variables)  # B, L, D
        out_transformers2 = self.forward_encoder_preprocessing(out_transformers2, lead_times, variables)  # B, L, D

        out_transformers = self.aggregattion(out_transformers0, out_transformers1, out_transformers2)
        out_transformers = self.forward_encoder(out_transformers, lead_times, variables) 
