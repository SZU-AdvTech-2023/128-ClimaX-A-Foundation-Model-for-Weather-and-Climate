...
#change:
class myNpyReader(IterableDataset):
    def __init__(
        self,
        file_list,
        start_idx,
        ...
        for (inp, out, lead_times, variables, out_variables) in self.dataset:
            ...
            for i in range(2, inp.shape[0]):
                if self.region_info is not None:
                    ...
                    yield torch.concat((self.transforms(inp[i - 2]), self.transforms(inp[i - 1]), self.transforms(inp[i])), dim=0), self.output_transforms(out[i]), lead_times[i], variables, out_variables
